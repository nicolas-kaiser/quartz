...
`config.rs` gives `Strategy`/`Tactic`/`Restrictions` file I/O: `from_json_str`/`from_yaml_str`/`to_json_string`/`to_yaml_string`/`load(path)`/`save(path)` (extension dispatch .json/.yaml/.yml; `ConfigError` plain enum). YAML crate is `yaml_serde` (the YAML org's maintained serde-yaml continuation) cargo-renamed to `serde_yaml`. Wire format is tuned for hand-authoring: `Dimension.dim_type` is flattened + internally tagged (`type: quadratic` / `type: linear` + `score_key`), `Sense` lowercase (old capitalized values accepted via aliases); `ScoreBound`/`Exclusion` stay externally tagged — YAML tag syntax `bound: !min 5.0`, `- !by_asset XYZ` (internal tagging fails on newtype-of-scalar variants). The Strategy **file API normalizes dimension weights** (like the builder); raw serde does not. No load-time validation beyond parsing — `compile()` validates at solve time. `flatten` disables unknown-key rejection inside dimensions.
